...
1. Merge all the assessment waves in a dataset with a number of rows equal to wavesBYparticipants. 
2. Create as many new variables as needed, as per the OPAL handbook for WP2.
3. Transform variables that require transformation outside the OBIBA environment (sensitive data).
...
I delete ~ 200 observations that are tests or simply not valid 
...
Now I need to go again to the raw datasets
...
I actually need only the 2020 information, because there is no previous data. I create the income dataset in case it can get expanded soon.
...
df_str <- 
  ...
  mutate(SD03 = case_when(SD03 == "0" ~ "2",
         ...
         healthcare_worker = 1,
         profession = case_when(TR14 == "1" ~ "4", # Check "14"
                                TR14 == "2" ~ "4",
                                TR14 == "3" ~ "4",
                                TR14 == "4" ~ "4",
                                TR14 == "5" ~ "4",
         ...
         children_household = if_else(SD08 == "0",
                                      "2",
                                      SD08), #TODO: check w Pap
         children_n = case_when(SD08a == "0" ~ "1",
                                SD08a == "1" ~ "2",
                                SD08a == "2" ~ "3",
                                SD08a == "3" ~ "3",
                                SD08a > "3" ~ "4"), #TODO: check w Pap
         living_alone = if_else(SD07 == "0",
                                "1",
                                "2"),
         household_size = case_when(SD07 == "1" ~ "1",
                                    SD07 == "2" ~ "2",
                                    SD07 == "3" ~ "3",
                                    SD07 == "4" ~ "3",
                                    SD07 > "4" ~ "4")
...
```{r eval = FALSE}
df_str %>% 
  mutate(age_cat = case_when(EDAD < 18 ~ "below 18 years",
                             EDAD > 17 & EDAD < 45 ~ "18-44",
                             EDAD > 44 & EDAD < 65 ~ "45-64",
                             EDAD > 64 & EDAD < 95 ~ "65-94",
                             EDAD > 94 ~ "95 years or older",
                             ...
                             EDADV2 > 44 & EDADV2 < 65 ~ "45-64",
                             EDADV2 > 64 & EDADV2 < 95 ~ "65-94",
                             EDADV2 > 94 ~ "95 years or older")) -> df_str # see above
```
...
vv_respond_opal %>% 
  select(...1) %>% 
  filter(!...1 %in% colnames(df_str)) #TODO: check w Pap
```
