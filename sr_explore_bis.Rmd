...
ds <- 
  ...
  mutate(
    ...
    brs_total = select(., c(re57:re63)) %>% rowSums()/6, #FIXME: denom n not 6
...
sr_2 <- 
  ...
  select(sd03,
         ...
         ra50, # soc support at work
         ra51, # soc support outside
         mp25ea, # trust government
         mp25fb) %>%  # trust workplace
  mutate(
    across(
      .cols = c(ra50, ra51, mp25ea, mp25fb),
      .fns = ~as.numeric(as.character(.))
    )
  ) %>% 
  map_dfr(.,
...
srl_1 <- 
  ...
  add_column(outcome = "SR_long") %>% 
  add_column(wave = "long")

# I won't do wave 3 because it requires work. SR_rel same results
